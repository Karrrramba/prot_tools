...
hyd_vals <- unlist(stringr::str_extract_all(hyd, "[\\d+|\\.|\\-]+"))
hyd_names <- unlist(stringr::str_extract_all(hyd, "([a-zA-Z]+)"))
hydropath_ref <- setNames(hyd_vals, hyd_names)
saveRDS(hydropath_ref, file = "aa_hydropathy.RData")
...
## mass calculation
# accepts U for selenocysteine and O for pyrrolysine
aa_masses <- "
...
saveRDS(aa_dict, file = "aa_dict.RData")
aa_mw_mono <- setNames(aa_masses_mono, aa_code_1)
saveRDS(aa_mw_mono, file = "aa_mw_mono.RData")
aa_mw_avg <- setNames(aa_masses_avg, aa_code_1)
saveRDS(aa_mw_avg, file = "aa_mw_avg.RData")
